...
func printUsage() {
	fmt.Println("Usage:")
	fmt.Println("  Indexing:  textindex -c index -i <input_file.txt> -s <chunk_size> -o <index_file.idx> [-n <ngram_size>] [--chars]")
	...
	fmt.Println("  -n <size>       N-gram size (default: 1)")
	fmt.Println("  --chars         Use character-based n-grams instead of words")
}
